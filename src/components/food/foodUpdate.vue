<template>
    <div>
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item  :to="{ path: '/company' }">食品管理</el-breadcrumb-item>
...
<script>
    ...
    export default {
        ...
        created(){
          //取出路由跳转传递参数名是id的值
          this.ruleForm.foodid = this.$route.params.id
          this.getData();
        },
        methods: {
            //根据ID加载记录
            getData(){
                let url = `http://localhost:8081/food/get/${this.ruleForm.foodid}`
            ...
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {//验证成功
                       //提交数据到服务器
                       let url = 'http://localhost:8081/food/update'
